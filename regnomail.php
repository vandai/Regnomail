...
Class RegNoMail {
    ...
    function rgm_registration_error($errors, $sanitized_user_login, $user_email){
        ...
        if($rgm_fullname != "" AND $rgm_fullname == "1"){
            if ( empty( $_POST['first_name'] ) )
                $errors->add( 'first_name_error', __('<strong>ERROR</strong>: You must include a first name.','vnetware') );
            if ( empty( $_POST['last_name'] ) )
                $errors->add( 'last_name_error', __('<strong>ERROR</strong>: You must include a Last name.','vnetware') );
        }
        if($rgm_password != "" AND $rgm_password == "1"){
            if ( $_POST['password'] !== $_POST['repeat_password'] ) {
                    $errors->add( 'passwords_not_matched', __("<strong>ERROR</strong>: Passwords must match.",'vnetware') );
            }
            if ( strlen( $_POST['password'] ) < 8 ) {
                    $errors->add( 'password_too_short', __("<strong>ERROR</strong>: Passwords must be at least eight characters long",'vnetware') );
            }
        }
    ...
    function rgm_settings(){
        ...
        echo "<span><h2 style='padding-top:10px;'>" . __( 'Regnomail Settings') . "</h2></span>  ";
        ?>
<script>
        ...
        $("form#rgm_settings_form").submit();
                    ...
                    <form method="post" name="rgm_settings_form" id="rgm_settings_form" action="options.php">
                        ...
                        <table class="form-table">
                            <tbody>
                                <tr valign="top"><th scope="row">Email Settings</th>
                                    <td>
                                        <fieldset>
                                            <legend class="screen-reader-text"><span>Email Settings</span></legend>
                                            ...
                                            Disable the automatic welcome email sent to users</label>
                                            <br />
                                            ...
                                            Disable new user notification email to admin</label>
                                            <br />
                                            ...
                                            Disable user password change notification email to admin</label>
                                            <br />
                                        </fieldset>
                                    </td>
                                </tr>
                                <tr valign="top"><th scope="row">Registration Form</th>
                                    <td>
                                        <fieldset>
                                            ...
                                            Show First & Last Name form</label>
                                            <br />
                                            ...
                                            Show Password form</label>
                                            <br />
                                        </fieldset>
                                    </td>
                                </tr>
                                <tr valign="top"><th scope="row">Custom Login Logo</th>
                                    ...
                                    <input id="rgm_upload_button" type="button" class="button rgm_upload_button" value="<?php _e( 'Select Image', 'rgm' ); ?>" />
                        ...
                        <p class="submit">
                            ...
                            <input type="button" id="rgm_reset_default" class="button-primary" value="<?php _e('Reset To Default') ?>" />
                        </p>
                    </form>
